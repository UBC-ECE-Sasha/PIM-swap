 ...
 */
#define WRAM_PER_TASKLET ((WRAM_SIZE / NR_TASKLETS) - (2 * OUT_BUFFER_LENGTH) - STACK_SIZE_DEFAULT)
...
static inline uint32_t read_uint32(struct in_buffer_context *input, uint32_t offset)
{
	...
	if ((offset - input->curr) < (SEQREAD_CACHE_SIZE - 4)) {
		offset -= input->curr;
		return (WRAM_HEAP_PTR(input->ptr)[offset] |
				(WRAM_HEAP_PTR(input->ptr)[offset + 1] << 8) |
				(WRAM_HEAP_PTR(input->ptr)[offset + 2] << 16) |
				(WRAM_HEAP_PTR(input->ptr)[offset + 3] << 24));
	}
	else {
		uint8_t data_read[16];
		mram_read(MRAM_PTR(&input->buffer[WINDOW_ALIGN(offset, 8)]), data_read, 16);
...
static void write_output_buffer(struct out_buffer_context *output, uint8_t *arr, uint32_t len)
{
	...
	while (len) {
		...
		to_write = MIN(OUT_BUFFER_LENGTH - curr_index, len);
...
static void copy_output_buffer(struct in_buffer_context *input, struct out_buffer_context *output, uint32_t len)
{
	...
	while (len) {
		...
		if (curr_index >= OUT_BUFFER_LENGTH) {
			...
			mram_write(output->append_ptr, MRAM_PTR(&output->buffer[output->append_window]), OUT_BUFFER_LENGTH);
			output->append_window += OUT_BUFFER_LENGTH;
			curr_index -= OUT_BUFFER_LENGTH;
		}

		to_copy = MIN(OUT_BUFFER_LENGTH - curr_index, len);
...
static inline int32_t find_match_length(struct in_buffer_context *input, uint32_t s1, uint32_t s2, uint32_t s2_limit)
{
	...
	// Remaining bytes
	x = read_uint32(input, s1 + matched) ^ read_uint32(input, s2);
