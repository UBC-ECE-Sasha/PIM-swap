					...
					bool stack_is_up)
{
	...
	printk("Returning status from stack %d\n", status);
	return status;
}
...
static uint32_t dpu_reset_internal_state(struct dpu_rank_t *rank)
{
	...
	printk("Inside dpu_reset_internal state\n");
	nr_cis = rank->region->addr_translate.desc.topology
			 .nr_of_control_interfaces;
	nr_dpus_per_ci = rank->region->addr_translate.desc.topology
				 .nr_of_dpus_per_control_interface;
	...
	printk("Running firmware update\n");
	/* DPU program should be located at /lib/firmware */
	int status_kern = request_firmware_direct(&reset_program, "internalStateReset", &rank->dev);
	if (status_kern < 0) {
		printk("Firmware is the problem errcode %d\n", status_kern);
		return DPU_ERR_NO_SUCH_FILE;
	}
	printk("Got the firmware\n");
	...
	printk("Running select all\n");
	FF(ufi_select_all(rank, &mask));
	printk("Running iram write\n");
	FF(ufi_iram_write(rank, mask, iram_array, 0,
			  internal_state_reset_size));

	printk("Running thread boot\n");
						     ...
						     uint8_t *expected)
{
	...
	if (!timeout) {
		printk("Timeout waiting for result to be correct %s\n", __FUNCTION__);
